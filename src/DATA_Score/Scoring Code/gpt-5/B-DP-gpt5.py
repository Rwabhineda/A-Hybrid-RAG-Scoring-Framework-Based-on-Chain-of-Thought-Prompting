...
SIMILARITY_THRESHOLD = CFG.get("similarity_threshold", 1.0)
...
CACHE_DIR = CFG.get("cache_dir", "clause_cache")
    ...
    for pattern in json_patterns:
        ...
        if match:
            try:
                ...
                json_str = json_str.replace('[', '').replace(']', '') # Handle scores in brackets
...
class BatchScorer:
                ...
                try:
                    ...
                    async with self.session.post(OPENAI_API_URL, headers=headers, json=payload, **kwargs) as response:
                        if response.status == 200:
                            ...
                            print(f"WARNING: API rate limit reached. Retrying after {wait_time} seconds...")
                    ...
                    print(f"WARNING: API request exception on attempt {attempt+1}: {e}")
                    if attempt < 2:
    ...
    async def process_clause(self, clause: Dict) -> Tuple:
        ...
        if cached_result:
            print(f"INFO: Cache hit for ID: {clause['id']}")
        ...
        if is_exception(scores, llm_output):
            print(f"WARNING: Exceptional scores detected for ID: {clause['id']}. Triggering second attempt.")
            ...
            if scores2:
                ...
                if is_exception(scores2, llm_output2):
                    print(f"WARNING: Re-scoring still exceptional for ID: {clause['id']}. Applying conservative fallback scores.")
...
async def main():
    print(" Initializing batch scoring system...")
    ...
    print(f"INFO: Found {len(clauses)} total clauses. {len(completed_ids)} already processed. {len(remaining_clauses)} remaining.")
    ...
    with open(OUTPUT_FILE, "a", encoding="utf-8") as fout, \
        ...
        num_batches = math.ceil(len(remaining_clauses) / BATCH_SIZE)
        for i, batch_start in enumerate(range(0, len(remaining_clauses), BATCH_SIZE)):
            batch_end = min(batch_start + BATCH_SIZE, len(remaining_clauses))
            batch = remaining_clauses[batch_start:batch_end]
            
            print(f"\n--- Processing Batch {i + 1}/{num_batches} (Clauses {batch_start + 1}-{batch_end} of remaining) ---")
            ...
            for result in results:
                if isinstance(result, Exception):
                    print(f"ERROR: An exception occurred while processing a clause: {result}")
                    continue
                    
                clause, llm_output, scores = result
                
                if not llm_output or any(v is None for v in scores.values()):
                    print(f"ERROR: API call failed for clause ID: {clause['id']}. Skipping.")
                    continue
            ...
            print(f"Batch complete in {batch_time:.2f}s. | Progress: {total_processed}/{len(remaining_clauses)} ({(total_processed/len(remaining_clauses))*100:.1f}%) | Avg time: {avg_time_per_item:.2f}s/clause | Exceptions: {exception_count}")
            fout.flush()
            log_fout.flush()
    ...
    print(f"  Clauses processed in this run: {total_processed}")
    print(f"  Average time per clause: {avg_time_per_clause:.2f} seconds")
    print(f"  Exceptional scores encountered: {exception_count} ({exception_rate:.1f}%)")
    print(f"  Output data saved to: {OUTPUT_FILE}")
    print(f"  Exception log saved to: {EXCEPTION_LOG_FILE}")
